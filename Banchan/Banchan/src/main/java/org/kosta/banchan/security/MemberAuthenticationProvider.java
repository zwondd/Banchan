...
public class MemberAuthenticationProvider implements AuthenticationProvider {
	 ...
	 * AuthenticationException -실제 인증 처리 - 규칙 1. 파라미터로 전달받은 Authentication 객체에 대해
	 * 인증처리를 지원하지 않으면 null을 리턴한다. 2. Authentication 객체를 이용한 인증에 실패하면
	 * AuthenticationException 발생시킨다. 3. 인증에 성공하면, 인증 정보를 담은 Authentication 객체를 만들어
	 * return 한다.
	 */
	/**
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		 ...
		 */
		/*
		 * 비밀번호 암호화를 이용할 경우 이용자가 로그인 폼에서 입력한 비밀번호와 DB로부터 가져온 암호화된 비밀번호를 비교한다
		 */
